...
async function main() {
  ...
  if (process.platform === "win32") {
    mksnapshotBinPath = require.resolve(
  ...
  if (process.platform !== "darwin") {
    const matchingDirs = crossArchDirs
      .map((dir) => `${mksnapshotBinPath}/${dir}`)
      .filter((dir) => fs.existsSync(dir));
    for (const dir of matchingDirs) {
      if (fs.existsSync(`${mksnapshotBinPath}/gen/v8/embedded.S`)) {
        await mkdirp(`${dir}/gen/v8`);
        fs.copyFileSync(
          `${mksnapshotBinPath}/gen/v8/embedded.S`,
          `${dir}/gen/v8/embedded.S`
        );
      }
    }
  }
  ...
  const res = childProcess.execFileSync(
    require.resolve(
      `electron-mksnapshot/bin/mksnapshot${
